...
class TestProductBarcodeSequence(TransactionCase):
    ...
    def test_workflow_04_manual_barcode_generation_action(self):
        ...
        # Step 1: Create product without barcode in enabled category
        product = self._create_product_in_category(
            "Manual Test Product", self.category_electronics
        )
    ...
    def test_workflow_05_bulk_barcode_generation(self):
        ...
        # Step 1: Create multiple products without barcodes
        products = self.env["product.product"]
        for i in range(3):
            product = self._create_product_in_category(
                f"Bulk Product {i+1}", self.category_electronics
            )
            products += product
            self.assertFalse(
        ...
        action_result = products.action_generate_barcode()

        # Step 3: Verify all products have barcodes
        for i, product in enumerate(products):
            self.assertTrue(product.barcode, f"Product {i+1} should have barcode")
            self._verify_barcode_format(product.barcode, "123456")
        ...
        # Step 5: Verify action reports correct count
        self.assertIn("3 products", action_result["params"]["message"])
    ...
    def test_workflow_08_gtin_check_digit_calculation(self):
        ...
        test_cases = [
            ("123456000001", 8),  # From documentation
            ("123456000002", 5),  # From documentation
            ("123456000003", 2),  # From documentation
        ]
    ...
    def test_workflow_09_computed_field_can_generate_barcode(self):
        ...
        # Step 2: Test product with existing barcode
        product.write({"barcode": "1234560000018"})
        self.assertFalse(
            product.can_generate_barcode,
